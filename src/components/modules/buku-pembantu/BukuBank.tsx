// src/components/modules/buku-pembantu/BukuBank.tsx
"use client";

import { useState } from "react";
import { useBukuBank } from "@/hooks/useBukuPembantu";
...
export function BukuBank() {
  const [bulan, setBulan] = useState<number | undefined>(undefined);
  const { data: rows = [], isLoading } = useBukuBank(bulan);
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-base">Buku Pembantu Bank</h2>
          </div>
          <Select
        ...
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">
            Belum ada transaksi bank{bulan !== undefined ? ` di ${BULAN_LABELS[bulan]}` : ""}.
          </p>
        ) : (
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={r.id} className={`hover:bg-muted/30 ${r.pengeluaran > 0 ? "bg-red-50/30 dark:bg-red-950/10" : ""}`}>
                    ...
                    <td className="border px-2 py-1 text-right text-xs text-green-700 font-medium">{r.penerimaan > 0 ? formatRupiah(r.penerimaan) : "—"}</td>
                    <td className="border px-2 py-1 text-right text-xs text-red-600 font-medium">{r.pengeluaran > 0 ? formatRupiah(r.pengeluaran) : "—"}</td>
