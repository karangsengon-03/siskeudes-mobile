// src/components/modules/pelaporan/PDFBukuBank.tsx
// Laporan 6: Buku Pembantu Bank — Portrait A4

import React from "react";
import { Document, Page, Text, View } from "@react-pdf/renderer";
import { sharedStyles as S, COLOR } from "./pdfStyles";
import { formatRupiah, formatTanggal } from "@/lib/utils";
import type { BukuBankRow } from "@/hooks/useBukuPembantu";
...
export function PDFBukuBank({ tahun, dataDesa, rows, bulan }: Props) {
    ...
    <Document>
      <Page size="A4" orientation="portrait" style={S.page}>
        <View style={S.docHeader}>
          <Text style={S.docTitle}>BUKU PEMBANTU BANK</Text>
        ...
        <View style={S.table}>
          ...
          {rows.length === 0 ? (
            <View style={S.tableRow}>
              <Text style={[S.tableCell, { flex: 1, color: COLOR.textMuted }]}>Belum ada transaksi bank</Text>
            </View>
          ) : (
            rows.map((row, i) => (
